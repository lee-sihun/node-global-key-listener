...
__declspec(dllexport) LRESULT CALLBACK KeyboardEvent(int nCode, WPARAM wParam, LPARAM lParam)
{
    ...
    if ((nCode == HC_ACTION) && ks)
    {
        ...
        if (haltPropogation(false, ks == down, key.vkCode, key.scanCode, zeroPoint, isExtended))
        {
...
__declspec(dllexport) LRESULT CALLBACK MouseEvent(int nCode, WPARAM wParam, LPARAM lParam)
{
    ...
    if (nCode >= 0 && pMouseStruct != NULL && ks && vCode)
    {
        // Stop propogation if needed using stdio messaging. 1st param is casted lPARAM.
        // Returning 1 from this function will halt propogation
        if (haltPropogation(true, ks == down, vCode, vCode, pMouseStruct->pt, false))
        {
            return 1;
        }
    }
...
bool haltPropogation(bool isMouse, bool isDown, DWORD vkCode, DWORD scanCode, POINT location, bool isExtended = false)
{
    ...
    if (isMouse)
    {
        printf("%s,%s,%i,%i,%ld,%ld,%i,0\n", "MOUSE", (isDown ? "DOWN" : "UP"), vkCode, scanCode, location.x, location.y, curId);
    }
    else
    {
        printf("%s,%s,%i,%i,%ld,%ld,%i,%i\n", "KEYBOARD", (isDown ? "DOWN" : "UP"), vkCode, scanCode, location.x, location.y, curId, isExtended ? 1 : 0);
    }
    fflush(stdout);
